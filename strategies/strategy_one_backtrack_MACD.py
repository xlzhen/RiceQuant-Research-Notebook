'''
https://blog.csdn.net/Quant_/article/details/49785933
https://blog.csdn.net/a19990412/article/details/79724372
'''
...
SMOOTHPERIOD = 9
OBSERVATION = 200 
MACD_END_DATE = datetime.datetime.now() - timedelta(days=2) 
MACD_START_DATE = MACD_END_DATE - timedelta(days=100)

def MACD(stock_id):
    prices = get_price(stock_id, MACD_START_DATE, MACD_END_DATE, frequency = "1d", fields=['volume','close'], adjust_type='none')
    ...
    if (macd[-1] - signal[-1] < 0 and macd[-2] - signal[-2] < 0) or (macd[-1] - signal[-1] > 0 and macd[-2] - signal[-2] > 0):
        return "无"
                    ...
                    if p['volume'][num_of_trade_days-1]*2 > p['volume'][num_of_trade_days] and p['volume'][num_of_trade_days] > p['volume'][num_of_trade_days-1]:
                            ...
                            if avg > 0: 
                                    if avg*2.7 > p['volume'][num_of_trade_days] and p['volume'][num_of_trade_days] > avg*1.2:
                                        ...
                                        if count > 0:  
                                            list_s.append([stock_id, status, stock_max_in_past_180,current_close,macd])  #加入输出表#
                                            number_of_stocks = number_of_stocks + 1
          
    df = pd.DataFrame(list_s) 
    df.rename(columns={0: '股票代码', 1:'是否超过', 2:'最高价', 3:'最低价',4:'MACD'}, inplace=True)                                                      
    df.to_csv('跳空高开留缺口—当天版MACD.csv') 
    print("done")          
